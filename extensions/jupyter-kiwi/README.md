...
- `build.zig`: small Zig regression target wired against the active Kiwi source root
- `bridge/include/kiwi_bridge.h`: shared C ABI for persistent Kiwi sessions, loaded from the active Kiwi source root
- `python/runtime/src/kiwi_array/bridge.py`: main-owned Python bridge wrapper consumed by this kernel
...
KIWI_ROOT="$(cd ../.. && pwd)"
if [ ! -f "$KIWI_ROOT/src/kiwi_bridge.zig" ]; then
  for candidate in "$KIWI_ROOT"/implementations/kiwi-*; do
    if [ -f "$candidate/src/kiwi_bridge.zig" ]; then
      KIWI_ROOT="$(cd "$candidate" && pwd)"
      break
    fi
  done
fi
PYTHONPATH=src uv run --managed-python --no-project \
  --with-editable "$KIWI_ROOT/python/runtime" \
  --with-editable "$KIWI_ROOT/python/host" \
  --with ipykernel \
  --with jupyter_client \
  --with jupyter_core \
  python -m kiwi_array_jupyter_kernel.install --user
```
...
The install helper builds the official bridge in the active Kiwi source root's
`zig-out/lib` if needed.
...
PYTHONPATH=src uv run --managed-python --no-project \
  ...
  --with jupyter_client \
  --with jupyter_core \
  pytest tests/test_kiwi_jupyter_kernel.py
zig build test
```
...
- The loader looks for packaged wheel libraries first and then the source checkout's `zig-out/lib`.
