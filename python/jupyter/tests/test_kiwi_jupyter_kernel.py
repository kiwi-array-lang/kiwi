...
PYTHON_ROOT = JUPYTER_ROOT.parent
KIWI_ROOT = PYTHON_ROOT.parent
PACKAGE_SRC = JUPYTER_ROOT / "src"
RUNTIME_SRC = PYTHON_ROOT / "runtime" / "src"
for src in (PACKAGE_SRC, RUNTIME_SRC):
    if str(src) not in sys.path:
        sys.path.insert(0, str(src))

from kiwi_jupyter_kernel.binding import (
    LEGACY_LIB_ENV_VAR,
    LIB_ENV_VAR,
    KiwiEvalResult,
    experiment_root,
...
from kiwi_jupyter_kernel.execution import (
    CellExecutionError,
    CellOutput,
    execute_cell,
...
from kiwi_jupyter_kernel.install import build_bridge, build_kernel_spec, find_logo_path, install_kernel, write_kernel_logos
from kiwi_jupyter_kernel.kernel import _display_data_for_output
...
def test_build_kernel_spec_uses_module_entrypoint(monkeypatch):
    ...
    assert spec["argv"] == ["python", "-m", "kiwi_jupyter_kernel", "-f", "{connection_file}"]
...
def test_logo_helpers_find_current_kernel_icon_and_copy_kernel_assets(tmp_path):
    ...
    assert logo_path == JUPYTER_ROOT / "src" / "kiwi_jupyter_kernel" / "assets" / "kernel-logo.png"
